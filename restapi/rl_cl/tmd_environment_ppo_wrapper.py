...
class PPOFriendlyWrapper(gym.Wrapper):
    ...
    def get_episode_metrics(self) -> dict:
        """Delegate to wrapped environment's get_episode_metrics method"""
...
def make_ppo_friendly_env(
    earthquake_data,  # Can be file path (str) or earthquake data array (np.ndarray)
    ...
    Args:
        earthquake_data: Either:
            - Path to earthquake CSV file (str), OR
            - Earthquake acceleration data array (np.ndarray)
        max_force: Maximum control force (N)
        normalize_obs: Whether to normalize observations
        clip_reward: If set, clip rewards to ±clip_reward
        dt: Time step (only used if earthquake_data is array)
        earthquake_name: Name for environment (only used if earthquake_data is array)

    Returns:
        Wrapped environment optimized for PPO
    """
    from .tmd_environment_adaptive_reward import make_improved_tmd_env, ImprovedTMDBuildingEnv

    # Check if earthquake_data is a file path or array
    if isinstance(earthquake_data, str):
        # File path - use the helper function
        base_env = make_improved_tmd_env(earthquake_data, max_force=max_force)
    else:
        # Array - create environment directly
        base_env = ImprovedTMDBuildingEnv(
